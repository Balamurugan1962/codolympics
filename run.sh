...
#
#   ./run.sh up        start Postgres, the sandbox, the judge and the web app (dev mode)
...
#   ./run.sh test      judge tests (real sandbox), web unit tests
...
WEB="$ROOT/web"; JUDGE="$ROOT/judge"; WORKER="$ROOT/worker"
...
ensure_env() {
  ...
  sed -e "s|^BETTER_AUTH_SECRET=$|BETTER_AUTH_SECRET=$(openssl rand -hex 32)|" \
      ...
      -e "s|^JUDGE_URL=.*|JUDGE_URL=http://127.0.0.1:$JUDGE_PORT|" \
...
up_web() {
  ...
  pnpm --dir "$WEB" db:migrate > "$LOGS/migrate.log" 2>&1 || die "migrations failed, see $LOGS/migrate.log"
...
cmd_up() {
  ensure_env; up_postgres; up_sandbox; up_judge; up_web
  echo
...
cmd_down() {
  say "stopping web and judge-api"
  kill_pidfile web; kill_pidfile judge-api
  for p in $WEB_PORT $JUDGE_PORT; do local pid; pid="$(listening $p)"; [ -n "$pid" ] && kill "$pid" 2>/dev/null || true; done
...
cmd_reset() {
  ...
  pnpm --dir "$WEB" db:migrate > "$LOGS/migrate.log" 2>&1
  say "clearing uploaded problems"; rm -rf "$JUDGE/problems"/*
  warn "restart the web app so it recreates the admin account: ./run.sh down && ./run.sh up"
}
...
cmd_test() {
  ...
  say "web unit tests (separate contest_test database)"
  up_postgres
  docker compose -f "$WEB/docker-compose.yml" exec -T postgres psql -U contest -d postgres -tAc "select 1 from pg_database where datname='contest_test'" | grep -q 1 \
    || docker compose -f "$WEB/docker-compose.yml" exec -T postgres psql -U contest -d postgres -q -c "create database contest_test" >/dev/null
  ( cd "$WEB" && DATABASE_URL=postgres://contest:contest@localhost:5432/contest_test pnpm db:migrate >/dev/null 2>&1 && DATABASE_URL=postgres://contest:contest@localhost:5432/contest_test pnpm test 2>&1 | grep -E "Tests|Test Files" )
}

cmd_smoke() {
  ensure_env; cmd_down >/dev/null; cmd_reset >/dev/null 2>&1 || true
  up_postgres; up_sandbox; up_judge; up_web
