...
export default function Phase1LeaderboardPage() {
  ...
  return (
    <PageBody className="animate-fade-in">
      <PageHeader title="Phase 1 standings" description="Ties go to the earlier finish." />
      {!board ? <TableSkeleton rows={8} cols={4} /> : board.mode === "hidden" && state?.viewer.role === "participant" ? (
        ...
        <div className="space-y-3">
          ...
          <Section padded={false}>
            <Table>
              <TableHeader><TableRow><TableHead className="w-16 text-right num">Rank</TableHead><TableHead>Participant</TableHead><TableHead className="text-right num">Points</TableHead><TableHead className="hidden sm:table-cell">Submitted</TableHead><TableHead className="w-32"><span className="sr-only">Decision</span></TableHead></TableRow></TableHeader>
              <TableBody>{board.standings.map((s) => (
                <TableRow key={s.participant_id} data-state={s.participant_id === me ? "selected" : undefined}>
                  <TableCell className="font-semibold text-right num">{s.rank ? `#${s.rank}` : "—"}</TableCell>
                  ...
                  <TableCell className="font-semibold text-right num">{s.points}{s.provisional && <span className="ml-1 text-faint" title="some items not yet graded">*</span>}</TableCell>
