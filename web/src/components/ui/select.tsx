...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      data-size={size}
      className={cn(
        "flex w-fit items-center justify-between gap-2 rounded-md border border-input bg-card px-3 py-2 text-[13px] font-medium whitespace-nowrap shadow-xs transition-[color,border-color,box-shadow] outline-none hover:border-line-2 focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 disabled:cursor-not-allowed disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-destructive/20 data-[placeholder]:text-muted-foreground data-[size=default]:h-9 data-[size=sm]:h-8 *:data-[slot=select-value]:line-clamp-1 *:data-[slot=select-value]:flex *:data-[slot=select-value]:items-center *:data-[slot=select-value]:gap-2 dark:bg-input/30 dark:hover:bg-input/50 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 [&_svg:not([class*='text-'])]:text-muted-foreground",
        className
      )}
      {...props}
    >
...
/**
 * Base UI splits Radix's Content into Positioner (placement), Popup (the box)
 * and List (the scrolling area that holds the items). Radix's `position` prop
 * has no counterpart — Base UI always positions against the trigger — so it is
 * accepted and ignored rather than made a breaking change for call sites.
 *
 * The popup is at least as wide as the trigger, which is what
 * `--radix-select-trigger-width` used to buy.
 */
...
/**
 * The shape a filter or a short enum actually needs: a value, a change handler
 * and a list. Radix's Select is six components deep, which is right for a
 * country picker and wrong for "show: all / sold / unsold" repeated in nine
 * toolbars.
 */
      ...
      <SelectContent>
        {options.map((o) => (
          <SelectItem key={o.value} value={o.value}>
            {o.label}
            {o.hint && <span className="ml-1 text-faint">{o.hint}</span>}
          </SelectItem>
        ))}
      </SelectContent>
    </Select>
