...
function TableHead({ className, ...props }: React.ComponentProps<"th">) {
  return (
    <th
      ...
      className={cn(
        "h-8 px-3 text-left align-middle text-[10.5px] font-semibold tracking-[0.08em] whitespace-nowrap text-faint uppercase first:pl-0 last:pr-0 [&:has([role=checkbox])]:pr-0",
        className,
      )}
      {...props}
    />
...
function TableCell({ className, ...props }: React.ComponentProps<"td">) {
  return (
    <td
      ...
      className={cn("h-10 px-3 align-middle first:pl-0 last:pr-0 [&:has([role=checkbox])]:pr-0", className)}
      {...props}
    />
  );
}
