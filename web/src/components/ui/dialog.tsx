 ...
 * Dialog, on Base UI.
 *
 * Three differences from the Radix version this replaces, all of them forced:
 *
 *   Overlay is called Backdrop, and Content is called Popup. The class strings
 *   moved across unchanged; only the element names differ.
 *
 *   Open state is expressed as `data-open` / `data-closed` rather than
 *   `data-state="open"`, so every animation variant is written `data-[open]:`
 *   instead of `data-[state=open]:`. Same animations, different selector.
 *
 *   `onOpenChange` is called with `(open, details)` rather than `(open)`. Call
 *   sites that take one argument keep working, so none of them changed.
 *
 * Focus trapping is the reason this library exists for us: every dialog here
 * gates a destructive, audit-logged action behind a typed reason. Base UI traps
 * by default (`modal`), restores focus to the trigger on close, and closes on
 * Escape.
 */
