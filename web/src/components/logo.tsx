 ...
 * rather than rounded, on an indigo tile. The square cuts are what stop it reading
...
export function Logo({ inverse = true, size = 28, className = "" }: { inverse?: boolean; size?: number; className?: string }) {
  return (
    <span className={`inline-flex items-center ${className}`} style={{ gap: size * 0.3 }}>
      ...
      >
        {/* On white the wordmark is small bold text, so it takes the deep
            indigo that clears AA; on the navy chrome the bright one is legible. */}
        Cod<span className={inverse ? "text-brand-bright" : "text-brand-deep"}>olympics</span>
      </span>
    </span>
  );
}
