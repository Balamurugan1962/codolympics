 ...
 * rather than rounded, on a blue tile. The square cuts are what stop it reading
...
export function Logo({ inverse = true, size = 28, className = "" }: { inverse?: boolean; size?: number; className?: string }) {
  return (
    <span className={`inline-flex items-center ${className}`} style={{ gap: size * 0.3 }}>
      ...
      >
        {/* On white the wordmark is small bold text, so it takes the deep blue
            that clears AA; on the navy chrome the bright one is legible. */}
