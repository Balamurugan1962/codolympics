...
def implemented_routes(client) -> set[tuple[str, str]]:
    """(path, method) for every route the app serves, read from the schema
    FastAPI generates, so routers nested in routers are all seen."""
    return {
        (path, method)
...
def test_every_documented_route_exists(spec, client):
    implemented = implemented_routes(client)
...
def test_no_undocumented_routes(spec, client):
    ...
    for path, method in implemented_routes(client):
        assert (path, method) in documented, f"{method.upper()} {path} is implemented but undocumented"
